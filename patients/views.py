...
def patient_dashboard(request):
    """Patient dashboard view with payment integration"""
    try:
...
def book_appointment(request):
    """Book new appointment with payment integration"""
    patient = get_object_or_404(Patient, user=request.user)
            ...
            try:
                if Doctor:
                    ...
                    if Appointment:
                        ...
                        # Pass appointment to template for payment
                        context = {
...
def appointment_detail(request, appointment_id):
    """View appointment detail with payment option"""
    patient = get_object_or_404(Patient, user=request.user)
...
def patient_prescriptions(request):
    ...
    return render(request, 'patients/prescriptions.html', {'prescriptions': page_obj, 'patient': patient})
